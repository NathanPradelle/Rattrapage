...
export default function CreateHarvestTour({ warehouses, auth }) {
    ...
    useEffect(() => {
        if (selectedWarehouse && selectedDate && selectedPeriod) {
            axios.get(route('api.harvest-requests.filter'), {
                params: {
    ...
    const handleAssistantChange = (volunteerId, checked) => {
        if (checked && volunteerAssistants.length < 2) {
            ...
            setVolunteerAssistants(volunteerAssistants.filter(id => id !== volunteerId));
        }
    };
    ...
    return (
        <AdminLayout user={auth.user}>
            ...
            <div className="container mx-auto p-4">
                <h2 className="text-2xl font-bold mb-4">Créer une Tournée de Récolte</h2>
                <form onSubmit={handleSubmit} className="space-y-4">
                    ...
                    <div>
                        <h3 className="font-semibold">Quantité Totale:
                            <span
                                className={totalQuantity < 50 || totalQuantity > 500 ? 'text-red-500' : ''}> {totalQuantity}</span>
                        </h3>
                        <table className="table-auto w-full mt-2 border border-gray-700">
                            ...
                            {harvestRequests.map(request => (
                                <tr key={request.id}>
                                    <td className="border border-gray-700 px-4 py-2">{request.user_name}</td>
                                    <td className="border border-gray-700 px-4 py-2">
                                        {request.building_number} {request.street}, {request.city}, {request.postal_code}
                                    </td>
                                    <td className="border border-gray-700 px-4 py-2">{request.quantity}</td>
                                    <td className="border border-gray-700 px-4 py-2">
                                        {request.note && (
                                            <button
                                                type="button"
                                    ...
                                    <td className="border border-gray-700 px-4 py-2">
                                        <input
                                            type="checkbox"
                                            value={request.id}
                                            onChange={(e) => handleRequestChange(request, e.target.checked)}
                                        />
                                    </td>
                                </tr>
                            ))}
                                ...
                                <tr key={volunteer.id}>
                                    ...
                                    <td className="border border-gray-700 px-4 py-2">
                                        <input
                                            type="checkbox"
                                            value={volunteer.id}
                                            onChange={(e) => handleAssistantChange(volunteer.id, e.target.checked)}
